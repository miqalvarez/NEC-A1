    ...
    for epoch in range(1, self.n_epochs):
      ...
      for pat in training_set:
        ...
        for neuron in range(self.n[self.L - 1]):
          self.errors_train[epoch][1] += pow(self.xi[self.L - 1][neuron] - y_train[pat], 2)
      ...
      if self.validation_set != None:
        ...
        for pat in validation_set:
          ...
          for neuron in range(self.n[self.L - 1]):
            self.errors_valid[epoch][1] += pow(self.xi[self.L - 1][neuron] - y_valid[pat], 2)
...
nn = MyNeuralNetwork(layers, fact[0], derivatives[0],perc=0.8, epochs=100, learning_rate=0.01, momentum=0.9)  

# read csv with numpy
data_array = np.loadtxt('A1-turbine-normalized.csv', delimiter=',', skiprows=1)
X = data_array[:, :-1]
y = data_array[:, -1]
